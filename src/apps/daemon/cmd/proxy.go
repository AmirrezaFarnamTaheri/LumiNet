...
var proxyCmd = &cobra.Command{
	Use:   "proxy",
	Short: "Evaluate Proxies (Workflow 3: Parse, test, score, and rank proxies)",
	Long:  `Evaluate Proxies workflow supports importing subscriptions, parsing into ASTs, batch validation, quality scoring, and configuration generation.`,
}
...
func runProxyGenerate(cmd *cobra.Command, args []string) error {
	baseURI := args[0]
	parsedBase, err := proxyconfig.ParseProxyURI(baseURI)
	if err != nil {
	...
	ips, err := diagnostics.GenerateCdnIPs(ranges, sampleRate)
	if err != nil {
		return err
	}
...
func runProxySubscribe(cmd *cobra.Command, args []string) error {
	...
	proxies, err := sub.Fetch(ctx, subURL)
	if err != nil {
...
func loadProxyList(args []string) ([]*proxyconfig.ProxyConfig, error) {
	var uris []string
	...
	var proxies []*proxyconfig.ProxyConfig
	for _, uri := range uris {
		cfg, err := proxyconfig.ParseProxyURI(uri)
		if err != nil {
...
func exportClash(proxies []*proxyconfig.ProxyConfig) ([]byte, error) {
...
func exportSingBox(proxies []*proxyconfig.ProxyConfig) ([]byte, error) {
...
func exportCSVProxies(proxies []*proxyconfig.ProxyConfig) ([]byte, error) {
