...
import (
	...
	"github.com/maybeknott/luminet/internal/integrations/notifier"
...
func initScheduler(cfg *config.Config) (*scheduler.Runner, error) {
	...
	// Periodic Network Health Audit (Runs every 5 minutes in background if enabled)
	if cfg.NetworkHealthAuditEnabled {
		err := runner.Register(&scheduler.Job{
