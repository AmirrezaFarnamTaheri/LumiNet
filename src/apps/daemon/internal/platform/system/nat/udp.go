...
func (u *UDP) WriteTo(buf []byte, local net.Addr, remote net.Addr) (int, error) {
	...
	ip := IPv4Packet(u.buf[:])
	ip.SetHeaderLen(IPv4HeaderSize)
	ip.SetTotalLength(IPv4HeaderSize + UDPHeaderSize + uint16(len(buf)))
	...
	udp := UDPPacket(ip.Payload())
	udp.SetLength(UDPHeaderSize + uint16(len(buf)))
...
func (u *UDP) handleUDPPacket(ip IPv4Packet, pkt UDPPacket) {
	var c *call
