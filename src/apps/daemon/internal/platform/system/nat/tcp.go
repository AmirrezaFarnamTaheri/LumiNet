package nat

import (
	"net"
	"net/netip"
	"time"
)

type TCP struct {
	listener *net.TCPListener
	portal   netip.Addr
	table    *table
}

func (t *TCP) Accept() (net.Conn, error) {
	c, err := t.listener.AcceptTCP()
	if err != nil {
		return nil, err
	}

	addr := c.RemoteAddr().(*net.TCPAddr)
	tup := t.table.findTupleByPort(uint16(addr.Port))
	if !addr.IP.Equal(t.portal.AsSlice()) || tup == zeroTuple {
		_ = c.Close()
		return nil, net.InvalidAddrError("unknown remote addr")
	}

	_ = c.SetKeepAlive(false)

	return &TranslatedConn{
		Conn:  c,
		tuple: tup,
	}, nil
}

func (t *TCP) Close() error {
	return t.listener.Close()
}

func (t *TCP) Addr() net.Addr {
	return t.listener.Addr()
}

func (t *TCP) SetDeadline(time time.Time) error {
	return t.listener.SetDeadline(time)
}

type TranslatedConn struct {
	net.Conn
	tuple tuple
}

func (c *TranslatedConn) LocalAddr() net.Addr {
	return &net.TCPAddr{
		IP:   c.tuple.from.Addr().AsSlice(),
		Port: int(c.tuple.from.Port()),
	}
}

func (c *TranslatedConn) RemoteAddr() net.Addr {
	return &net.TCPAddr{
		IP:   c.tuple.to.Addr().AsSlice(),
		Port: int(c.tuple.to.Port()),
	}
}

func (c *TranslatedConn) RawConn() (net.Conn, bool) {
	return c.Conn, true
}

func (c *TranslatedConn) Unwrap() (net.Conn, bool) {
	return c.RawConn()
}
