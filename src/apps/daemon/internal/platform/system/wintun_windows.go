...
func (s *wintunSession) ReceivePacket() ([]byte, error) {
	...
	if err := validateWintunPacketSize(int(packetSize)); err != nil {
		return nil, err
	}
	ptr := *(*unsafe.Pointer)(unsafe.Pointer(&r0))
	packet := unsafe.Slice((*byte)(ptr), packetSize)
	return packet, nil
}
...
func (s *wintunSession) AllocateSendPacket(packetSize int) ([]byte, error) {
	if err := validateWintunPacketSize(packetSize); err != nil {
		return nil, err
	}
