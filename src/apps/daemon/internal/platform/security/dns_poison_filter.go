...
type DnsPoisonFilter struct {
	bogusIPs map[string]bool
	dropped  uint64
	accepted uint64
	mu       sync.RWMutex
}

func NewDnsPoisonFilter() *DnsPoisonFilter {
	f := &DnsPoisonFilter{
		bogusIPs: make(map[string]bool),
	}
	knownBogus := []string{
	...
	for _, ip := range knownBogus {
		f.bogusIPs[ip] = true
	}
	return f
}

func (f *DnsPoisonFilter) IsBogus(ip net.IP) bool {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if ip == nil {
		return false
	}
	return f.bogusIPs[ip.String()]
}

func (f *DnsPoisonFilter) FilterIPs(ips []net.IP) []net.IP {
	f.mu.Lock()
	defer f.mu.Unlock()

	var clean []net.IP
	for _, ip := range ips {
		if f.bogusIPs[ip.String()] {
			f.dropped++
		} else {
			f.accepted++
...
func (f *DnsPoisonFilter) Stats() (uint64, uint64) {
	f.mu.RLock()
	defer f.mu.RUnlock()
