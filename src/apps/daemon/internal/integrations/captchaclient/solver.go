...
func (cs *CaptchaSolver) SolveCaptcha(ctx context.Context, method string, siteURL string, siteKey string, extraParams map[string]string) (string, error) {
	...
	policy := remoteaction.DefaultPolicy("captcha.task.create", remoteaction.SingleAttempt)
	outcome, err := remoteaction.Do(ctx, cs.HTTPClient, policy, func(ctx context.Context, _ int) (*http.Request, error) {
		req, err := http.NewRequestWithContext(ctx, http.MethodPost, inURL, strings.NewReader(formData.Encode()))
		if err != nil {
	...
	if resp == nil {
		return "", errors.New("captcha task creation completed without response")
	}
	defer resp.Body.Close()

	bodyBytes, err := readBoundedCaptchaResponse(resp.Body)
	if err != nil {
		return "", err
	}
	...
	for {
		...
		case <-ticker.C:
			...
			pollBytes, err := readBoundedCaptchaResponse(respPoll.Body)
			respPoll.Body.Close()
			if err != nil {
				continue
			}
