...
func (n *Notifier) SendWebhook(ctx context.Context, alert *Alert) error {
	...
	policy := remoteaction.DefaultPolicy("notifier.webhook.send", remoteaction.SingleAttempt)
	outcome, err := remoteaction.Do(ctx, n.httpClient, policy, func(ctx context.Context, _ int) (*http.Request, error) {
	...
	if resp == nil {
		return fmt.Errorf("webhook send completed without response")
	}
