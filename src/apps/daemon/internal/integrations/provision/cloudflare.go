...
func (c *CFClient) doReq(ctx context.Context, method, url string, body []byte, contentType string) ([]byte, error) {
	...
	respBytes, err := io.ReadAll(io.LimitReader(resp.Body, maxCloudflareResponseBytes+1))
	if err != nil {
		return nil, err
	}
	if len(respBytes) > maxCloudflareResponseBytes {
		return nil, fmt.Errorf("Cloudflare response exceeds %d bytes", maxCloudflareResponseBytes)
	}

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return respBytes, formatCloudflareAPIError(resp.StatusCode, respBytes, resp.Header.Get("Retry-After"))
	}

	return respBytes, nil
}

func (c *CFClient) doMutation(ctx context.Context, action string, class remoteaction.SafetyClass, method, requestURL string, body []byte, contentType string, reconcile remoteaction.ReconcileFunc) ([]byte, error) {
	policy := remoteaction.DefaultPolicy(action, class)
	policy.RateLimitScope = "provider.cloudflare"
	outcome, err := remoteaction.Do(ctx, c.client, policy, func(ctx context.Context, _ int) (*http.Request, error) {
		var bodyReader io.Reader
		if body != nil {
			bodyReader = bytes.NewReader(body)
		}
	...
	if outcome.Response == nil {
		return nil, fmt.Errorf("remote action %q completed without response or reconciliation", action)
	}
	defer outcome.Response.Body.Close()

	respBytes, err := io.ReadAll(io.LimitReader(outcome.Response.Body, maxCloudflareResponseBytes+1))
	if err != nil {
		return nil, err
	}
	if len(respBytes) > maxCloudflareResponseBytes {
	...
	if outcome.Response.StatusCode < 200 || outcome.Response.StatusCode >= 300 {
		return respBytes, formatCloudflareAPIError(outcome.Response.StatusCode, respBytes, outcome.Response.Header.Get("Retry-After"))
	}
	return respBytes, nil
}
...
func (c *CFClient) findDNSRecord(ctx context.Context, zoneID, name string) (cloudflareDNSRecord, bool, error) {
	searchURL := fmt.Sprintf(
		"https://api.cloudflare.com/client/v4/zones/%s/dns_records?type=A&name=%s",
		zoneID,
		url.QueryEscape(name),
	)
	respBytes, err := c.doReq(ctx, http.MethodGet, searchURL, nil, "")
	if err != nil {
	...
	if !searchRes.Success {
		return cloudflareDNSRecord{}, false, fmt.Errorf("Cloudflare DNS record lookup reported success=false")
	}
	if len(searchRes.Result) == 0 {
		return cloudflareDNSRecord{}, false, nil
	}
	return searchRes.Result[0], true, nil
}
...
func (c *CFClient) UpsertDNSRecord(ctx context.Context, zoneID, name, ip string, proxied bool) error {
	record, found, err := c.findDNSRecord(ctx, zoneID, name)
	if err != nil {
		return err
	}
	...
	if found {
		...
		_, err = c.doMutation(ctx, "cloudflare.dns.update", remoteaction.Idempotent, http.MethodPut, updateURL, body, "application/json", nil)
		return err
	}

	// Create is not blindly replayed. If the response is ambiguous, read back
	// the named record first and replay only when it is still absent.
	...
	reconcile := func(ctx context.Context) (bool, error) {
		reconciled, exists, err := c.findDNSRecord(ctx, zoneID, name)
		if err != nil {
			return false, err
		}
		if !exists {
		...
		if reconciled.Type != "A" || reconciled.Name != name || reconciled.Content != ip || reconciled.Proxied != proxied {
			return false, fmt.Errorf("DNS record %q appeared after ambiguous create but does not match desired state", name)
		}
		return true, nil
	}
	_, err = c.doMutation(ctx, "cloudflare.dns.create", remoteaction.ReconcileBeforeRetry, http.MethodPost, createURL, body, "application/json", reconcile)
	return err
}
...
func (c *CFClient) SetSSLModeStrict(ctx context.Context, zoneID string) error {
	...
	_, err = c.doMutation(ctx, "cloudflare.ssl.strict", remoteaction.Idempotent, http.MethodPatch, url, body, "application/json", nil)
	return err
}
...
func (c *CFClient) CreateOriginCertificate(ctx context.Context, hostnames []string, csrPEM string, validityDays int) ([]byte, error) {
	...
	respBytes, err := c.doMutation(ctx, "cloudflare.origin-certificate.create", remoteaction.SingleAttempt, http.MethodPost, url, body, "application/json", nil)
	if err != nil {
		return nil, err
	}
...
`, camouflageHost, cfg.D1DatabaseBinding, cfg.TargetHost, cfg.TargetPort) + "\n" + getDashboardHTMLJS()
	...
	_, err = client.doMutation(ctx, "cloudflare.worker.upload", remoteaction.Idempotent, http.MethodPut, uploadURL, body, contentType, nil)
	if err != nil {
	...
	_, err = client.doMutation(ctx, "cloudflare.worker.subdomain.enable", remoteaction.SingleAttempt, http.MethodPost, routeURL, subdomainBody, "application/json", nil)
	if err != nil {
