...
func (s *Server) CreateProxyScan(c *gin.Context) {
	...
	jobID, err := s.createAndStartJob(jobs.ProxyTestIntent{Proxies: req.Proxies, URLs: req.URLs, Timeout: req.Timeout, Concurrency: req.Concurrency, SpeedTest: req.SpeedTest, GeoIP: req.GeoIP, CoreType: req.CoreType, DNSResolver: req.DnsResolver})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusAccepted, ProxyScanResponse{ID: jobID, Status: string(jobs.JobStatusRunning), Progress: 0, CreatedAt: time.Now(), TotalProxies: len(req.Proxies)})
}
