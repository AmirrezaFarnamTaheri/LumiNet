...
import (
	...
	"github.com/maybeknott/luminet/internal/integrations/presets"
...
func TestPresetHandlers(t *testing.T) {
	...
	t.Run("GET /api/presets/scanner", func(t *testing.T) {
		...
		var presets []presets.ScanPreset
		if err := json.Unmarshal(w.Body.Bytes(), &presets); err != nil {
	...
	t.Run("GET /api/presets/serverless", func(t *testing.T) {
		w := httptest.NewRecorder()
		req, _ := http.NewRequest(http.MethodGet, "/api/presets/serverless", nil)
		router.ServeHTTP(w, req)
		...
		var presets []presets.ServerlessRoutingPreset
		if err := json.Unmarshal(w.Body.Bytes(), &presets); err != nil {
			t.Fatalf("failed to unmarshal Serverless presets: %v", err)
		}

		if len(presets) == 0 {
			t.Error("expected at least one Serverless preset")
		}
	})

	t.Run("GET /api/presets/cottendns", func(t *testing.T) {
		w := httptest.NewRecorder()
		req, _ := http.NewRequest(http.MethodGet, "/api/presets/cottendns", nil)
		router.ServeHTTP(w, req)
		...
		var presets []sub.CottenDnsPreset
		if err := json.Unmarshal(w.Body.Bytes(), &presets); err != nil {
			t.Fatalf("failed to unmarshal CottenDNS presets: %v", err)
		}

		if len(presets) == 0 {
			t.Error("expected at least one CottenDNS preset")
		}
	})
	...
	t.Run("GET /api/presets", func(t *testing.T) {
		...
		if _, ok := unified["serverless"]; !ok {
			t.Error("expected serverless key in unified presets response")
		}
		if _, ok := unified["cottendns"]; !ok {
			t.Error("expected cottendns key in unified presets response")
		}
	})
}
