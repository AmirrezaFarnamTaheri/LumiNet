...
import (
	...
	"github.com/maybeknott/luminet/internal/foundation/pingcache"
...
func (s *Server) GetSystemStatus(c *gin.Context) {
	...
	updater := dns.NewUpdater("cloudflare", "", "") // provider doesn't matter for GetPublicIP
	...
	evasionStatus := proxy.GetEvasionManager().Status()
	tunActive := system.GetTunRouterManager().IsRunning()

	upBytes, downBytes := trafficstats.GetEvasionTrafficStats()

	configMutationStats := config.MutationStats{}
	if s.configManager != nil {
		configMutationStats = s.configManager.MutationStats()
	}
	webSocketStats := WebSocketStats{}
	if s.hub != nil {
		webSocketStats = s.hub.Stats()
	}
	...
	c.JSON(http.StatusOK, SystemStatusResponse{
		...
		EvasionActive:         evasionStatus.Running,
		TunActive:             tunActive,
		...
		ConfigMutationRetry:   configMutationStats,
		RemoteMutationRetry:   remoteaction.DefaultExecutorStats(),
		WebSocketBackpressure: webSocketStats,
	})
}
...
var (
	globalPingCache *pingcache.PingCache
	pingOnce        sync.Once
)

func getPingCache() *pingcache.PingCache {
	pingOnce.Do(func() {
		globalPingCache = pingcache.New("./config/ping_cache.json")
	})
	return globalPingCache
}

// HandleSystemPing handles GET /api/system/ping
func (s *Server) HandleSystemPing(c *gin.Context) {
	host := c.Query("host")
	if host == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "host parameter is required"})
		return
	}
	portVal := c.DefaultQuery("port", "443")
	port, err := strconv.Atoi(portVal)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid port"})
		return
	}
	useCacheVal := c.DefaultQuery("use_cache", "true")
	useCache := useCacheVal == "true"

	cache := getPingCache()
	res := cache.GetDelay(c.Request.Context(), host, port, useCache)
