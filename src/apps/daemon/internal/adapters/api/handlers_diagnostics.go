...
func (s *Server) RunDiagnostic(c *gin.Context) {
	...
	jobID, err := s.createAndStartJob(jobs.DiagnosticIntent{Type: req.Type, Target: req.Target, Timeout: req.Timeout, Options: req.Options})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusAccepted, gin.H{"id": jobID, "status": string(jobs.JobStatusRunning), "progress": 0, "created_at": time.Now()})
}
...
func (s *Server) HandleGeoIPLookup(c *gin.Context) {
	...
	if err != nil {
		// Fallback to our reverse-engineered ipgeo cascade
		info, cascadeErr := diagnostics.DetectGeoInfo(c.Request.Context(), diagnostics.LookupOptions{
			SocksProxy: "", // standard direct query fallback
			MaxRetries: 2,
		})
		if cascadeErr != nil {
