...
// Stream event constants matching lumicore_abi.h
const (
	StreamEvtProbeResult uint16 = 1
	StreamEvtPoolUpdate  uint16 = 2
	StreamEvtScanDone    uint16 = 3
	StreamEvtError       uint16 = 255
)

// StreamEvent represents an asynchronous event emitted by the native core stream.
type StreamEvent struct {
	Type uint16 // STREAM_EVT_* constant
	Data []byte
}

// StreamProbeResultData mirrors lumicore StreamProbeResult payload.
type StreamProbeResultData struct {
	Target    string  `json:"target"`
	Port      uint16  `json:"port"`
	Open      bool    `json:"open"`
	LatencyMs float64 `json:"latency_ms"`
...
// StreamProgressData mirrors lumicore StreamProgress payload.
