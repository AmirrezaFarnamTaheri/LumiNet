...
func TestDiagnostics_SnifferDetect(t *testing.T) {
	pipeline := NewPipeline()
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	job := &DiagnosticJob{
		Type:    MetricSnifferDetect,
		Timeout: 3 * time.Second,
	}

	result, err := pipeline.Run(ctx, job)
	if err != nil {
		t.Fatalf("Failed to run SnifferDetect diagnostic: %v", err)
	}

	if !result.Success {
		t.Fatalf("SnifferDetect diagnostic run was unsuccessful: %s", result.RawOutput)
	}

	if _, ok := result.Metrics["sniffer_detected"]; !ok {
		t.Error("Expected 'sniffer_detected' metric to be present")
	}

	t.Logf("SnifferDetect Diagnostic raw output:\n%s", result.RawOutput)
}

func TestDiagnostics_ArpCache(t *testing.T) {
	pipeline := NewPipeline()
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	result, err := pipeline.Run(ctx, job)
	if err != nil {
		t.Fatalf("Failed to run ArpCache diagnostic: %v", err)
	}

	if !result.Success {
		t.Fatalf("ArpCache diagnostic run was unsuccessful: %s", result.RawOutput)
	}

	if _, ok := result.Metrics["host_count"]; !ok {
		t.Error("Expected 'host_count' metric to be present")
	}

	t.Logf("ArpCache Diagnostic raw output:\n%s", result.RawOutput)
}
