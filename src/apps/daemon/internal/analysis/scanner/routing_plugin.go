...
// ─── Routing Plugin Types ─────────────────────────────────────────────────
// Target: LumiNet scanner package routing plugin validation
...
// RoutingPluginDescriptor describes a single routing plugin entry point.
...
// RoutingPluginRegistry holds validated plugin descriptors.
type RoutingPluginRegistry struct {
	descriptors map[string]RoutingPluginDescriptor
}

// RoutingPluginConfig is the user-supplied configuration for a route.
...
// RoutingPluginConfigValidation is the result of validating a config.
...
// NewRoutingPluginRegistry builds a registry from validated descriptors.
...
// List returns all registered plugin descriptors sorted by PluginID.
...
// Get returns a descriptor by plugin ID.
...
func validateRoutingPluginDescriptor(d RoutingPluginDescriptor) error {
	...
	required := []string{
		d.PluginID, d.PluginType, d.DisplayName, d.Version,
		d.SourceURL, d.License, d.RouteType, d.CredentialMode,
		d.LocalAPIMode, d.SecretPolicy, d.DiagnosticLabel,
	}
