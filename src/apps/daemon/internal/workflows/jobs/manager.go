...
const (
	...
	// JobTypeDiagnostic is a configured diagnostic pipeline job.
	...
	// JobTypeIpDiscovery is an IP discovery sweep job.
	JobTypeIpDiscovery JobType = "ip_discovery"
	// JobTypeStreamScan is a reactive streaming scan connected to the native core pipeline.
	JobTypeStreamScan JobType = "stream_scan"
)
...
type Job struct {
	...
	// Config is the persisted/public job configuration (JSON). Secrets are redacted.
	Config string
	// Results is the serialized job results (JSON), populated on completion.
	Results string
	// Error is the error message if the job failed.
	Error string
	// RecoveredFrom is the immutable source job ID when this job was explicitly
	// requeued after an interrupted daemon execution. Empty means original job.
	RecoveredFrom string
	// intent is the normalized in-memory execution contract. It is never serialized.
	intent JobIntent
...
type JobManager struct {
	...
	ctx   context.Context
	// recoveryMu serializes check-and-create recovery operations so duplicate
	// confirmations cannot create simultaneous descendants of one source job.
...
func NewJobManager(ctx context.Context, db *store.DB) *JobManager {
	if ctx == nil {
		ctx = context.Background()
	}
	policy := scheduler.BudgetPolicy{
		MaxGlobalRunning: 10,
		MaxPerType: map[scheduler.JobType]int{
			scheduler.JobType("icmp_scan"):     3,
			scheduler.JobType("port_scan"):     3,
			scheduler.JobType("dns_scan"):      3,
			scheduler.JobType("tls_scan"):      3,
			scheduler.JobType("sni_scan"):      3,
			scheduler.JobType("proxy_test"):    3,
			scheduler.JobType("speed_test"):    2,
			scheduler.JobType("diagnostic"):    1,
			scheduler.JobType("vps_provision"): 1,
			scheduler.JobType("edge_deploy"):   1,
			scheduler.JobType("ip_discovery"):  1,
		},
		MaxQueueDepth: 1000,
	}
	sched := scheduler.NewScheduler(policy)
	sched.Start(ctx)
	...
	if db != nil {
		...
		if err == nil {
			for _, r := range records {
				...
				if sanitized, changed := scrubLegacyConfig(JobType(r.Type), r.Config); changed {
					r.Config = sanitized
...
// Stop cancels queued/running scheduler work and waits for daemon-owned jobs to exit.
...
// publishEvent safely reads the broadcaster under the lock and publishes an event.
// This prevents data races when broadcaster is read concurrently with SetBroadcaster.
func (m *JobManager) publishEvent(event JobEvent) {
	m.mu.RLock()
	b := m.broadcaster
	m.mu.RUnlock()
	if b != nil {
...
func (m *JobManager) persistJob(job *Job) {
	...
	jr := &store.JobRecord{
		...
		Results:       job.Results,
		Error:         job.Error,
		RecoveredFrom: job.RecoveredFrom,
	}
	job.mu.RUnlock()
...
// persistEvidence writes probe evidence when the manager was configured with a
// store. In-memory managers are valid for local previews and handler tests, so
// evidence persistence must be optional just like job-record persistence.
func (m *JobManager) persistEvidence(ctx context.Context, item *evidence.ProbeEvidence) error {
	if m.db == nil {
		return nil
	}
	return m.db.Save(ctx, item)
}

// CreateJob creates a queued job from a sealed typed execution intent.
// The normalized execution intent stays in memory; Config contains only the
// persistence/public representation and is safe for history/export.
func (m *JobManager) CreateJob(intent JobIntent) (string, error) {
	return m.createJob(intent, "")
}

func (m *JobManager) createJob(intent JobIntent, recoveredFrom string) (string, error) {
	normalized, err := normalizeIntent(intent)
	if err != nil {
		return "", err
	}
	config, err := persistedConfig(normalized)
	if err != nil {
		return "", err
	}
	m.mu.Lock()
	id := generateUUID()
	job := &Job{
		ID: id, Type: normalized.jobType(), Status: JobStatusQueued, Progress: 0,
		CreatedAt: time.Now(), Config: config, RecoveredFrom: recoveredFrom, intent: normalized,
	}
	m.jobs[id] = job
	m.mu.Unlock()
	m.persistJob(job)
	m.publishEvent(JobEvent{JobID: id, Type: "status_change", Data: string(JobStatusQueued), Timestamp: time.Now()})
	return id, nil
}
...
func (m *JobManager) StartJob(id string) error {
	...
	if job.intent == nil {
		job.mu.Unlock()
		m.mu.Unlock()
		return fmt.Errorf("job %s has no in-memory execution intent", id)
	}
	job.mu.Unlock()
	m.mu.Unlock()

	// The scheduler must accept ownership before the job becomes externally
	// visible as running. An admission gate prevents a fast dispatcher from
	// executing the job before that visible state transition is committed.
	admitted := make(chan struct{})
	cancelCtx, cancel := context.WithCancel(m.ctx)
	fjob := scheduler.NewFuncJob(job.ID, scheduler.JobType(job.Type), func(jc context.Context) error {
		select {
		case <-admitted:
		case <-jc.Done():
			return jc.Err()
		...
		mergedCtx, mergedCancel := context.WithCancel(cancelCtx)
		go func() {
			select {
			case <-jc.Done():
				mergedCancel()
		...
		defer mergedCancel()
		m.runJob(mergedCtx, job)
		return nil
	})
	if err := m.sched.Submit(fjob); err != nil {
		cancel()
		return fmt.Errorf("scheduler queue submission failed: %w", err)
	}
	...
	if job.Status != JobStatusQueued {
		...
		cancel()
		close(admitted)
		return fmt.Errorf("job changed to status %s during scheduler admission", job.Status)
	}
	...
	if m.db != nil {
		...
		if err == nil {
			...
			j := &Job{
				...
				Results:       jr.Results,
				Error:         jr.Error,
				RecoveredFrom: jr.RecoveredFrom,
			}
			return j, nil
		}
	}
...
// Broadcaster returns the event broadcaster attached to the job manager.
func (m *JobManager) Broadcaster() *Broadcaster {
	return m.GetBroadcaster()
}
