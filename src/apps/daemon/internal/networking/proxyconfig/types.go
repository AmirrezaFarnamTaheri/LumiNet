...
type ProxyConfig struct {
	...
	KCPObservedLossPercent float64 `json:"kcp_observed_loss_percent,omitempty"`
	KCPDataShards          int     `json:"kcp_data_shards,omitempty"`
	...
	KCPACKNoDelay          bool    `json:"kcp_ack_no_delay,omitempty"`
	KCPWriteDelay          bool    `json:"kcp_write_delay,omitempty"`
	KCPDSCP                int     `json:"kcp_dscp,omitempty"`
	KCPReadBufferBytes     int     `json:"kcp_read_buffer_bytes,omitempty"`
	KCPWriteBufferBytes    int     `json:"kcp_write_buffer_bytes,omitempty"`
	KCPPacketDuplication   int     `json:"kcp_packet_duplication,omitempty"`
	KCPRateLimitBPS        int64   `json:"kcp_rate_limit_bps,omitempty"`
	...
	// KCP*Set flags preserve explicit zero/false values from share links. They
	// are parser/runtime metadata and are intentionally not serialized to JSON.
	KCPObservedLossSet      bool `json:"-"`
	KCPDataShardsSet        bool `json:"-"`
	KCPParityShardsSet      bool `json:"-"`
	KCPNoDelaySet           bool `json:"-"`
	KCPIntervalSet          bool `json:"-"`
	KCPResendSet            bool `json:"-"`
	KCPNoCongestionSet      bool `json:"-"`
	KCPSendWindowSet        bool `json:"-"`
	KCPReceiveWindowSet     bool `json:"-"`
	KCPMTUSet               bool `json:"-"`
	KCPACKNoDelaySet        bool `json:"-"`
	KCPWriteDelaySet        bool `json:"-"`
	KCPDSCPSet              bool `json:"-"`
	KCPReadBufferSet        bool `json:"-"`
	KCPWriteBufferSet       bool `json:"-"`
	KCPPacketDuplicationSet bool `json:"-"`
	KCPRateLimitSet         bool `json:"-"`
	KCPJitterSet            bool `json:"-"`
	...
	// CipherSuites is the Xray TLS cipher-suite policy string.
	CipherSuites string `json:"cipher_suites,omitempty"`
	// ECHConfigList carries an Xray ECH config-list value when explicitly supplied.
	ECHConfigList string `json:"ech_config_list,omitempty"`
	// VerifyPeerCertByName enables Xray peer-certificate name verification.
	VerifyPeerCertByName string `json:"verify_peer_cert_by_name,omitempty"`
	// PinnedPeerCertSHA256 is an Xray raw peer-certificate SHA-256 pin.
	PinnedPeerCertSHA256 string `json:"pinned_peer_cert_sha256,omitempty"`
	// FinalMask is bounded raw JSON for Xray streamSettings.finalmask.
	FinalMask string `json:"final_mask,omitempty"`
	// XHTTPMode and XHTTPExtra preserve XHTTP transport-specific share fields.
	XHTTPMode  string `json:"xhttp_mode,omitempty"`
	XHTTPExtra string `json:"xhttp_extra,omitempty"`
	// SkipCertVerify records legacy share-link intent. Core builders must fail closed
	// rather than silently emitting removed/unsafe TLS options.
	SkipCertVerify bool `json:"skip_cert_verify,omitempty"`
	// Plugin is the SIP003 plugin name (SS).
	Plugin string `json:"plugin,omitempty"`
	// PluginOpts is the SIP003 plugin options.
	PluginOpts string `json:"plugin_opts,omitempty"`
	// Prefix is the Shadowsocks custom packet prefix.
	...
	// SpiderX is the Reality spiderX path/pattern.
	...
	// Hysteria2PortHopping preserves the share-link mport expression.
	Hysteria2PortHopping string `json:"hysteria2_port_hopping,omitempty"`
	...
	// AnyTLSIdleSessionCheckInterval is the AnyTLS idle-session maintenance cadence.
	AnyTLSIdleSessionCheckInterval string `json:"anytls_idle_session_check_interval,omitempty"`
	// AnyTLSIdleSessionTimeout is the AnyTLS idle-session expiration duration.
	AnyTLSIdleSessionTimeout string `json:"anytls_idle_session_timeout,omitempty"`
...
func normalizeProxyURI(uri string) string {
	// Real subscription feeds frequently HTML-escape query separators while
	// embedding proxy URIs in HTML/Telegram-derived text. Decode only the
	// observed ampersand entity so opaque credentials/payloads are untouched.
	return strings.ReplaceAll(uri, "&amp;", "&")
}

func queryBool(values url.Values, aliases ...string) bool {
...
func looseBoolString(value string) bool {
...
func looseBoolValue(values ...interface{}) bool {
...
func rejectAmbiguousProxyAuthority(uri string) error {
	...
	if strings.Contains(rest, ",") || strings.IndexFunc(rest, func(r rune) bool {
		return r == ' ' || r == '\t' || r == '\r' || r == '\n'
	}) >= 0 {
		return fmt.Errorf("ambiguous multi-authority proxy endpoint is not supported")
	}
	return nil
}
...
func ParseProxyURI(uri string) (*ProxyConfig, error) {
	uri = normalizeProxyURI(strings.TrimSpace(uri))
	if uri == "" {
	...
	if err := rejectAmbiguousProxyAuthority(uri); err != nil {
		return nil, err
	}
	...
	} else if strings.HasPrefix(lowered, "wireguard://") || strings.HasPrefix(lowered, "wg://") || strings.HasPrefix(lowered, "awg://") || strings.HasPrefix(lowered, "amneziawg://") || strings.HasPrefix(lowered, "warp://") || strings.HasPrefix(lowered, "purguard://") {
...
func (p *ProxyConfig) Validate() error {
	...
	case ProtocolShadowsocks:
		...
		if err := validateShadowsocks2022Key(p.Method, p.Password); err != nil {
			return err
		}
	...
	case ProtocolTUIC:
		...
		if mode := strings.ToLower(strings.TrimSpace(p.CongestionControl)); mode != "" {
			switch mode {
			case "cubic", "new_reno", "newreno", "bbr":
			default:
				return fmt.Errorf("unsupported TUIC congestion control %q", p.CongestionControl)
			}
		}
		if mode := strings.ToLower(strings.TrimSpace(p.UDPRelayMode)); mode != "" {
			switch mode {
			case "native", "quic":
			default:
				return fmt.Errorf("unsupported TUIC UDP relay mode %q", p.UDPRelayMode)
			}
		}
	case ProtocolKCP:
		if p.Password == "" {
...
func (p *ProxyConfig) ToURI() string {
	...
	case ProtocolVLESS:
		...
		q := url.Values{}
		if p.Transport != "" {
			q.Set("type", CanonicalXHTTPTransport(p.Transport))
		}
		...
		if p.CipherSuites != "" {
			q.Set("cs", p.CipherSuites)
		}
		if p.ECHConfigList != "" {
			q.Set("ech", p.ECHConfigList)
		}
		if p.VerifyPeerCertByName != "" {
			q.Set("vcn", p.VerifyPeerCertByName)
		}
		if p.PinnedPeerCertSHA256 != "" {
			q.Set("pcs", p.PinnedPeerCertSHA256)
		}
		if p.PublicKey != "" {
		...
		if p.FinalMask != "" {
			q.Set("fm", p.FinalMask)
		}
		if p.XHTTPMode != "" {
			q.Set("mode", p.XHTTPMode)
		}
		if p.XHTTPExtra != "" {
			q.Set("extra", p.XHTTPExtra)
		}
		if len(q) > 0 {
			u += "?" + q.Encode()
		}
	...
	case ProtocolHysteria2:
		...
		if p.UpMbps > 0 {
			q.Set("up", fmt.Sprintf("%d", p.UpMbps))
		}
		if p.DownMbps > 0 {
			q.Set("down", fmt.Sprintf("%d", p.DownMbps))
		}
		if p.Hysteria2PortHopping != "" {
			q.Set("mport", p.Hysteria2PortHopping)
		}
		if p.PinnedPeerCertSHA256 != "" {
	...
	case ProtocolKCP:
		...
		if p.KCPObservedLossSet || p.KCPObservedLossPercent != 0 {
			q.Set("loss", strconv.FormatFloat(p.KCPObservedLossPercent, 'f', -1, 64))
		}
		if p.KCPDataShardsSet || p.KCPDataShards != 0 {
			q.Set("data_shards", strconv.Itoa(p.KCPDataShards))
		}
		if p.KCPParityShardsSet || p.KCPParityShards != 0 {
			q.Set("parity_shards", strconv.Itoa(p.KCPParityShards))
		}
		if p.KCPNoDelaySet || p.KCPNoDelay != 0 {
			q.Set("nodelay", strconv.Itoa(p.KCPNoDelay))
		}
		if p.KCPIntervalSet || p.KCPInterval != 0 {
			q.Set("interval", strconv.Itoa(p.KCPInterval))
		}
		if p.KCPResendSet || p.KCPResend != 0 {
			q.Set("resend", strconv.Itoa(p.KCPResend))
		}
		if p.KCPNoCongestionSet || p.KCPNoCongestion != 0 {
			q.Set("nc", strconv.Itoa(p.KCPNoCongestion))
		}
		if p.KCPSendWindowSet || p.KCPSendWindow != 0 {
			q.Set("sndwnd", strconv.Itoa(p.KCPSendWindow))
		}
		if p.KCPReceiveWindowSet || p.KCPReceiveWindow != 0 {
			q.Set("rcvwnd", strconv.Itoa(p.KCPReceiveWindow))
		}
		if p.KCPMTUSet || p.KCPMTU != 0 {
			q.Set("mtu", strconv.Itoa(p.KCPMTU))
		}
		if p.KCPACKNoDelaySet || p.KCPACKNoDelay {
			q.Set("acknodelay", strconv.FormatBool(p.KCPACKNoDelay))
		}
		if p.KCPWriteDelaySet || p.KCPWriteDelay {
			q.Set("writedelay", strconv.FormatBool(p.KCPWriteDelay))
		}
		if p.KCPDSCPSet || p.KCPDSCP != 0 {
			q.Set("dscp", strconv.Itoa(p.KCPDSCP))
		}
		if p.KCPReadBufferSet || p.KCPReadBufferBytes != 0 {
			q.Set("readbuf", strconv.Itoa(p.KCPReadBufferBytes))
		}
		if p.KCPWriteBufferSet || p.KCPWriteBufferBytes != 0 {
			q.Set("writebuf", strconv.Itoa(p.KCPWriteBufferBytes))
		}
		if p.KCPPacketDuplicationSet || p.KCPPacketDuplication != 0 {
			q.Set("dup", strconv.Itoa(p.KCPPacketDuplication))
		}
		if p.KCPRateLimitSet || p.KCPRateLimitBPS != 0 {
			q.Set("rate", strconv.FormatInt(p.KCPRateLimitBPS, 10))
		}
		if p.KCPCompression != "" {
			q.Set("compression", p.KCPCompression)
		}
		if p.KCPJitterSet || p.KCPJitter {
			q.Set("jitter", strconv.FormatBool(p.KCPJitter))
		}
		if p.KCPJitterMin != 0 {
	...
	case ProtocolAnyTLS:
		...
		if p.AnyTLSIdleSessionCheckInterval != "" {
			q.Set("idle_session_check_interval", p.AnyTLSIdleSessionCheckInterval)
		}
		if p.AnyTLSIdleSessionTimeout != "" {
