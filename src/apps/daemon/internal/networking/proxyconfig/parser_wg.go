...
// parseWireGuard parses wireguard/wg/awg/amneziawg/warp/purguard share links.
func parseWireGuard(uri string) (*ProxyConfig, error) {
	normalized := uri
	isAmnezia := false
	lowered := strings.ToLower(uri)
	isPurguard := strings.HasPrefix(lowered, "purguard://")
	if strings.HasPrefix(lowered, "wg://") {
		...
		normalized = "wireguard" + uri[len("purguard"):]
	}
	...
	if preSharedKey == "" {
		preSharedKey = q.Get("pre_shared_key")
	}
	if preSharedKey == "" {
