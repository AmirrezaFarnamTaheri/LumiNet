...
func TestTorBridgeConfiguration(t *testing.T) {
	...
	if !strings.Contains(content, "AvoidDiskWrites 1") {
		t.Errorf("Expected privacy-preserving AvoidDiskWrites 1, got:\n%s", content)
	}
	if strings.Contains(content, "GeoIPFile ") || strings.Contains(content, "GeoIPv6File ") {
		t.Errorf("runtime torrc must not point at nonexistent temp-directory GeoIP files, got:\n%s", content)
	}
