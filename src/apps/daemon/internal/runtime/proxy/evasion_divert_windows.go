	...
	go func() {
		...
		for {
			...
			default:
				...
				srcIP, dstIP, srcPort, dstPort, seq, isSyn := parseTCPHandshake(packetBytes)
				if isSyn {
...
func StartBypassSniffer(conn *rawBypassConn) error {
	...
	if err := modWinDivert.Load(); err != nil {
		return fmt.Errorf("load WinDivert for raw bypass: %w", err)
	}
	...
	if handle == ^uintptr(0) {
		return fmt.Errorf("open WinDivert raw-bypass handle")
	}
