...
type evasionTunnelConn struct {
	...
	// Session-level write fragmentation
	sessionFrag           bool
...
func (c *evasionTunnelConn) writeRaw(b []byte) (int, error) {
	...
	if err == nil && n > 0 {
		trafficstats.AddUploadBytes(uint64(n))
	}
	return n, err
}
...
func (c *evasionTunnelConn) Read(b []byte) (int, error) {
	n, err := c.Conn.Read(b)
	if err == nil && n > 0 {
		trafficstats.AddDownloadBytes(uint64(n))
	}
	return n, err
}
...
func (c *evasionTunnelConn) Write(b []byte) (int, error) {
	...
	if c.firstWrite {
			...
			if host, portStr, err := net.SplitHostPort(remoteAddr); err == nil {
				...
				if portVal > 0 {
					...
					if portVal == 80 {
						...
						// Use the same bounded, realistic padded ClientHello as diagnostics.
						var buildErr error
						payloadBytes, buildErr = tlsdecoy.BuildPaddedClientHello(decoySNI)
						if buildErr != nil {
							return 0, fmt.Errorf("build fake TLS ClientHello: %w", buildErr)
						}
						payloadLen = len(payloadBytes)
						fakePayloadHex = hex.EncodeToString(payloadBytes)
					}
					...
					if okLocal && okRemote {
						...
						if synSeq, found := TakeConnSeq(localTCP.IP, localPort, remoteTCP.IP, remotePort); found {
							hasSeqInfo = true
							...
							ackVal = synSeq + 1
						} else if c.outOfWindowEnabled {
							return 0, fmt.Errorf("out-of-window fake injection requires a fresh captured SYN sequence for the exact TCP four-tuple")
						} else {
							// TTL-limited decoy mode preserves its historical best-effort fallback.
							seqVal = 1000
							ackVal = 1
						}
					}
					...
					if okLocal && okRemote {
						srcIP := localTCP.IP
						dstIP := remoteTCP.IP
						if c.ipSpoofingEnabled {
							if decoy := net.ParseIP(c.ipSpoofingDecoyIP); decoy != nil {
								srcIP = decoy
							}
							if realDst := net.ParseIP(c.ipSpoofingDstReal); realDst != nil {
								dstIP = realDst
							}
						}
					...
					if injectErr != nil {
						...
						targetHost := host
						if c.ipSpoofingEnabled && c.ipSpoofingDstReal != "" {
							targetHost = c.ipSpoofingDstReal
						}

						if err := bridge.InjectFakePacket(targetHost, portVal, uint32(c.fakePacketTtl), flagsPtr, seqPtr, ackPtr, fakePayloadHex); err != nil {
							return 0, fmt.Errorf("fake packet injection unavailable: %w", err)
						}
					}
				}
			}
		...
		if c.mutateHost || c.mutateHeaderSpace || c.mutateMethod || c.mutateAbsoluteUri || c.httpPadding > 0 || c.residentialCloaking {
			payload = normalizeHttpHeaders(payload, c.customUserAgent, c.mutateMethod, c.mutateAbsoluteUri, c.httpPadding, c.residentialCloaking, c.mutateHost, c.mutateHeaderSpace)
		}

		if c.residentialCloaking {
			payload = EmulateHTTP2Preface(payload, c.customUserAgent)
		}
	}
	...
	// Randomized Multi-Fragment splitting
	if c.randomMultiSplit && len(payload) > 2 {
		numFrags := c.numFragments
		if numFrags < 2 {
			numFrags = 2
		}
		if numFrags > len(payload) {
			numFrags = len(payload)
		}

		// Generate random indices to split
		indices := make([]int, numFrags-1)
		for i := 0; i < numFrags-1; i++ {
			for {
				idx := 1 + rand.Intn(len(payload)-1)
				unique := true
	...
	// Precision SNI multi-fragment splitting
