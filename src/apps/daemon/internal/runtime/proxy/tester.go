...
//
// tester.go implements the proxy testing engine,
...
type TestConfig struct {
	...
	// MullvadURL is the URL to check for Mullvad VPN exit status. If empty, the check is skipped.
	MullvadURL string
}
...
type TestResult struct {
	...
	// Mullvad indicates whether the exit IP belongs to Mullvad VPN infrastructure.
	Mullvad bool
	// MullvadServer is the Mullvad server name if Mullvad is true.
	MullvadServer string
}
...
func (t *ProxyTester) Start(ctx context.Context, proxies []*proxyConfig) error {
	...
	if _, err := t.coreManager.FindBinary(); err != nil {
		t.mu.Unlock()
		return fmt.Errorf("failed to locate core binary: %w", err)
	}
	t.inProgress = true
		...
		if err == nil {
			...
			if err == nil {
				...
				if resp.StatusCode == 200 {
					if body, readErr := readBoundedProxyHTTPBody(resp.Body, maxProxyControlHTTPBodyBytes, "GeoIP response"); readErr == nil {
						var geo GeoInfo
...
// StartBatch is an alias for Start for batch-oriented callers.
func (t *ProxyTester) StartBatch(ctx context.Context, proxies []*proxyConfig) error {
	return t.Start(ctx, proxies)
}

// mullvadResponse is the JSON shape returned by the Mullvad connectivity API.
type mullvadResponse struct {
	IP            string `json:"ip"`
	Mullvad       bool   `json:"mullvad"`
	MullvadServer string `json:"mullvad_server"`
}

// testSingleOnPortInternal tests a proxy on socksPort.
// When skipSpawn is true the core manager launch is skipped and the function
// tests connectivity directly against an already-listening socksPort.
func (t *ProxyTester) testSingleOnPortInternal(ctx context.Context, proxy *proxyConfig, socksPort int, skipSpawn bool) *TestResult {
	result := &TestResult{
		Proxy:  proxy,
		Status: "failed",
	}
	...
	if !skipSpawn {
		inst, err := t.coreManager.RunTempInstance(proxy, socksPort)
		if err != nil {
			result.Status = "error"
			result.Error = fmt.Sprintf("failed to start core instance: %v", err)
			return result
	...
	result.Latency = sum / float64(len(latencies))
	result.Status = "working"

	// Mullvad check
	if t.config.MullvadURL != "" {
		mReq, err := http.NewRequestWithContext(ctx, "GET", t.config.MullvadURL, nil)
		if err == nil {
			mReq.Header.Set("User-Agent", "LumiNet/1.0")
			mResp, err := client.Do(mReq)
			if err == nil {
				if body, readErr := readBoundedProxyHTTPBody(mResp.Body, maxProxyControlHTTPBodyBytes, "Mullvad status response"); readErr == nil {
					var mr mullvadResponse
					if json.Unmarshal(body, &mr) == nil {
						result.Mullvad = mr.Mullvad
						result.MullvadServer = mr.MullvadServer
					}
				}
				mResp.Body.Close()
			}
