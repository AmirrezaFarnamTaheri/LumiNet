...
func (l *LinuxPacketInjector) Start(ctx context.Context, listenPort int) error {
	...
	if err != nil {
		...
		if err != nil {
			return fmt.Errorf("open packet capture socket: %w", err)
		}
	}
	...
	go func(fd int) {
		...
		for {
			...
			default:
				...
				if isEth {
					if n >= 14+20 {
						...
						if ethType == 0x0800 {
							...
							copy(dMac[:], buf[0:6])
							copy(sMac[:], buf[6:12])
							RegisterMacInfo(dstIP.String(), MacInfo{SrcMAC: sMac, DstMAC: dMac, IfIndex: ifIndex})
						}
					}
				} else {
					ipPacket = buf[:n]
				...
				if len(ipPacket) >= 20 {
					srcIP, dstIP, srcPort, dstPort, seq, isSyn := parseTCPHandshake(ipPacket)
					if isSyn {
						RegisterConnSeq(srcIP, srcPort, dstIP, dstPort, seq)
					}
				}
			}
		}
...
func StartBypassSniffer(conn *rawBypassConn) error {
	fd, err := syscall.Socket(syscall.AF_INET, syscall.SOCK_RAW, syscall.IPPROTO_TCP)
	if err != nil {
		return fmt.Errorf("open raw bypass sniffer: %w", err)
	}
