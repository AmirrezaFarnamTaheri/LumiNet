...
func TestHandleSocksUDPAssociate(t *testing.T) {
	...
	go func() {
		...
		if buf[1] == 0x03 {
			HandleSocksUDPAssociate(conn, "127.0.0.1", expectedSocksUDPClientIP(conn.RemoteAddr(), "0.0.0.0"), 0)
		}
	}()
...
func TestSocksUDPClientAuthorityRejectsForeignFirstPacket(t *testing.T) {
	expectedIP := net.ParseIP("127.0.0.1")
	if socksUDPClientAllowed(&net.UDPAddr{IP: net.ParseIP("127.0.0.2"), Port: 44000}, expectedIP, 0) {
		t.Fatal("foreign first UDP packet was allowed to claim the SOCKS association")
	}
	if !socksUDPClientAllowed(&net.UDPAddr{IP: net.ParseIP("127.0.0.1"), Port: 44000}, expectedIP, 0) {
		t.Fatal("TCP-peer UDP source was rejected")
	}
}

func TestSocksUDPClientAuthorityHonorsDeclaredPort(t *testing.T) {
	expectedIP := net.ParseIP("127.0.0.1")
	if socksUDPClientAllowed(&net.UDPAddr{IP: expectedIP, Port: 44001}, expectedIP, 44000) {
		t.Fatal("undeclared UDP source port was accepted")
	}
	if !socksUDPClientAllowed(&net.UDPAddr{IP: expectedIP, Port: 44000}, expectedIP, 44000) {
		t.Fatal("declared UDP source endpoint was rejected")
	}
}

func TestReadSocksAddressConsumesAddressAndPort(t *testing.T) {
	payload := []byte{127, 0, 0, 1, 0x1f, 0x90} // 127.0.0.1:8080
	host, port, err := readSocksAddress(bytes.NewReader(payload), 0x01)
	if err != nil {
		t.Fatal(err)
	}
	if host != "127.0.0.1" || port != 8080 {
		t.Fatalf("got %s:%d, want 127.0.0.1:8080", host, port)
	}
}
