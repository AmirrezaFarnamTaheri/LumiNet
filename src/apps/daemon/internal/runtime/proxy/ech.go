...
var (
	...
	// It is secure by default; callers must explicitly opt into insecure verification.
...
// Cloudflare CIDR range specifications
...
// IsCloudflareIP returns true if the given IP address belongs to Cloudflare's networks.
...
// ResolveECHViaDoH resolves the ECH config list for a domain using DoH (DNS over HTTPS)
func ResolveECHViaDoH(ctx context.Context, domain string, resolverURL string) ([]byte, error) {
	if resolverURL == "" {
		resolverURL = "https://dns.quad9.net/dns-query" // default secure fallback
	}
	...
	req, err := http.NewRequestWithContext(ctx, "POST", resolverURL, bytes.NewReader(buf))
	if err != nil {
		return nil, err
	}
	...
	respBuf, err := readBoundedProxyHTTPBody(resp.Body, maxProxyDNSHTTPBodyBytes, "DoH response")
	if err != nil {
	...
	return nil, fmt.Errorf("no ECH config found in HTTPS records for %s", domain)
}
