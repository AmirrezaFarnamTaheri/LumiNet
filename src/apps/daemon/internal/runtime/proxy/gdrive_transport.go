...
// NewGDriveMailbox creates a credentialed Google Drive mailbox transport.
func NewGDriveMailbox(folderID, sessionID, accessToken string) (*GDriveMailbox, error) {
	if strings.TrimSpace(folderID) == "" {
		return nil, fmt.Errorf("gdrive mailbox requires a folder ID")
	}
	if strings.TrimSpace(sessionID) == "" {
		return nil, fmt.Errorf("gdrive mailbox requires a session ID")
	}
	if strings.TrimSpace(accessToken) == "" {
		return nil, fmt.Errorf("gdrive mailbox requires an access token")
	}
...
func (m *GDriveMailbox) ReadChunk(ctx context.Context, chunkIdx int) ([]byte, error) {
	...
	// Best-effort cleanup remains request-scoped; DELETE itself is safe to
	// replay because the desired terminal state is absence.
	deleteGoogleDriveFileBestEffort(ctx, m.HTTPClient, "gdrive.chunk.delete", m.AccessToken, fileID)
...
func (m *GDriveMailbox) VirtualConnection(ctx context.Context) net.Conn {
	return &gdriveConn{
		mailbox:  m,
		readBuf:  nil,
		chunkIdx: 0,
...
func (e *ZephyrEnvelope) Encode() ([]byte, error) {
	if len(e.Payload) > ZephyrMaxPayloadLen {
		return nil, fmt.Errorf("payload length %d exceeds maximum %d", len(e.Payload), ZephyrMaxPayloadLen)
	}
...
func DecodeZephyrEnvelope(data []byte) (*ZephyrEnvelope, error) {
	...
	if payLen > ZephyrMaxPayloadLen {
		return nil, fmt.Errorf("payload length %d exceeds maximum %d", payLen, ZephyrMaxPayloadLen)
	}
