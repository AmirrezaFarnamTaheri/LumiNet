...
func TestEvasionTunnelConn_RandomMultiSplit(t *testing.T) {
	c1, c2 := net.Pipe()
	defer c1.Close()
	defer c2.Close()
	...
	conn := &evasionTunnelConn{
		Conn:             c1,
		randomMultiSplit: true,
		numFragments:     5,
		delayMs:          1,
		firstWrite:       true,
	}

	payload := []byte("This is a payload split into randomized multiple chunks to bypass deep inspection.")
...
func TestEvasionTunnelConn_Write_MutateHeaderSpace(t *testing.T) {
	...
	go func() {
		receivedData := make([]byte, 0, 500)
		buf := make([]byte, 256)
		for !strings.HasSuffix(string(receivedData), "\r\n\r\n") && !strings.HasSuffix(string(receivedData), "\n\n") {
			n, err := c2.Read(buf)
			if err != nil {
				break
			}
	...
	recStr := string(received)
	if !strings.Contains(recStr, "Host :") && !strings.Contains(recStr, "Host:") && !strings.Contains(recStr, "hOsT:") && !strings.Contains(recStr, "\t") {
		t.Errorf("Expected mutated header, got: %q", recStr)
	}
}
...
func TestEvasionTunnelStopClosesActiveSocksSession(t *testing.T) {
	...
	initial := DefaultEvasionConfig()
	mgr.config.Store(&initial)

	cfg := DefaultEvasionConfig()
	cfg.Port = 0
	cfg.DnsForwarderEnabled = false
	cfg.SystemProxyEnabled = false
	cfg.FakePacketInject = false
	cfg.UpgenEnabled = false
	cfg.CovertMode = "direct"
	cfg.AutoSni = false
	if err := mgr.Start(&cfg); err != nil {
		t.Fatalf("start evasion tunnel: %v", err)
	}

	client, err := net.Dial("tcp", fmt.Sprintf("127.0.0.1:%d", mgr.Status().Port))
	if err != nil {
		mgr.Stop()
		t.Fatalf("dial evasion tunnel: %v", err)
	}
	defer client.Close()
	...
	request := []byte{0x05, 0x01, 0x00, 0x01, ip[0], ip[1], ip[2], ip[3], byte(targetAddr.Port >> 8), byte(targetAddr.Port)}
	...
	if _, err := io.ReadFull(client, response); err != nil {
		mgr.Stop()
		t.Fatalf("read SOCKS connect response: %v", err)
	}
	if response[1] != 0x00 {
		mgr.Stop()
	...
	case <-stopped:
	case <-time.After(2 * time.Second):
		t.Fatal("Stop did not quiesce active SOCKS session")
	}
	...
	if _, err := client.Read(one); err == nil {
		t.Fatal("active SOCKS client remained open after Stop")
	}
}
