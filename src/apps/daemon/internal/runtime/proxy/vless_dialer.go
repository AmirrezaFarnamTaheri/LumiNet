...
func (c *vlessWSConn) Read(b []byte) (int, error) {
	...
	for {
		...
		var ready bool
		msg, ready, err = c.vlessResponse.consume(next)
		if err != nil {
			return 0, err
		}
		if !ready {
...
func (c *vlessWSConn) LocalAddr() net.Addr                { return proxyWebSocketLocalAddr(c.Conn) }
func (c *vlessWSConn) RemoteAddr() net.Addr               { return proxyWebSocketRemoteAddr(c.Conn) }
func (c *vlessWSConn) SetDeadline(t time.Time) error      { return proxyWebSocketSetDeadline(c.Conn, t) }
