...
func TestEvasionTunnel_EdgeWiring(t *testing.T) {
	...
	err = mgr.Start(&EvasionConfig{
		Port:                    0,
		SplitBytes:              0,
		DelayMs:                 0,
		MutateHost:              false,
		MutateHeaderSpace:       false,
		AutoSni:                 false,
		SniSplitOffset:          0,
		Packets:                 "all",
		MinLength:               0,
		...
		SniSpoof:                "",
		ClientHelloPadding:      0,
		DelayJitter:             false,
		TcpWindowClamp:          0,
		CustomUserAgent:         "",
		CovertMode:              "edge",
		CovertServerlessUrl:     "socks5://" + socksAddr.String(),
		CovertDnsDomain:         "",
		CovertGsaUrl:            "",
		CovertGsaKey:            "",
		CovertGdocsFolderId:     "",
		...
		MutateSniCase:           false,
		MutateMethod:            false,
		MutateAbsoluteUri:       false,
		HttpPadding:             0,
		PreflightSignature:      "",
		PreflightDelayMs:        0,
		SessionFrag:             false,
		SessionFragProb:         0.0,
		SessionFragMinTotal:     0,
		SessionFragMaxTotal:     0,
		SessionFragMinChunk:     0,
		SessionFragMaxChunk:     0,
		SessionFragMinDelayMs:   0,
		SessionFragMaxDelayMs:   0,
		IpSpoofingEnabled:       false,
		IpSpoofingDecoyIP:       "",
		IpSpoofingDstReal:       "",
		OutOfWindowEnabled:      false,
		OutOfWindowSeqOffset:    0,
		DecoySniPool:            "",
		OobEnabled:              false,
		OobexEnabled:            false,
		AsyncReactorEnabled:     false,
		LossRate:                0.0,
		EmulatedLatency:         0,
		EmulatedJitter:          0,
		CircularCacheCap:        100,
		ShaperReadRate:          0,
		ShaperWriteRate:         0,
		...
		UpgenEnabled:            false,
		UpgenSeedHex:            "",
		UpgenEntropyMatch:       false,
		UpgenQuicExhaustionRate: 0,
		StegoEnabled:            false,
		StegoMode:               "",
		StegoDecoyImagePath:     "",
		StegoWebRTCSDPSpoof:     false,
	})
	if err != nil {
...
func TestEncodeSOCKS5TargetBounds(t *testing.T) {
	valid := []struct {
		host string
		port int
		atyp byte
	}{
		{host: "127.0.0.1", port: 443, atyp: 0x01},
		{host: "2001:db8::1", port: 53, atyp: 0x04},
		{host: "example.com", port: 65535, atyp: 0x03},
	}
	for _, tc := range valid {
		req, err := encodeSOCKS5Target(tc.host, tc.port)
		if err != nil {
			t.Fatalf("encodeSOCKS5Target(%q,%d): %v", tc.host, tc.port, err)
		}
		if len(req) < 6 || req[3] != tc.atyp {
			t.Fatalf("unexpected request for %q: %v", tc.host, req)
		}
		if got := int(binary.BigEndian.Uint16(req[len(req)-2:])); got != tc.port {
			t.Fatalf("port changed: got %d want %d", got, tc.port)
		}
	}
	...
	for _, tc := range []struct {
		...
		{host: "", port: 80},
		{host: "bad\x00host", port: 80},
		{host: strings.Repeat("a", 256), port: 80},
		{host: "example.com", port: 0},
		{host: "example.com", port: 65536},
	} {
		if _, err := encodeSOCKS5Target(tc.host, tc.port); err == nil {
			t.Fatalf("invalid target unexpectedly encoded: host=%q port=%d", tc.host, tc.port)
		}
	}
}
