...
func normalizeHttpHeaders(b []byte, customUserAgent string, mutateMethod bool, mutateAbsoluteUri bool, httpPadding int, residentialCloaking bool, mutateHost bool, mutateHeaderSpace bool) []byte {
	if len(b) < 16 {
		return b
	}
	isHttp := false
	...
	// Apply HTTP Method Casing and Padding Mutation if enabled
	if mutateMethod {
		reqParts := strings.SplitN(requestLine, " ", 2)
		if len(reqParts) == 2 {
			mutatedMethod := mutateStringCasing(reqParts[0])
			// Extra space (method padding): "GET  /path HTTP/1.0"
	...
	// Unix LF: \n instead of \r\n
	lineEnd := "\r\n"
	if mutateHeaderSpace {
		lineEnd = "\n"
	}

	reassembled.WriteString(requestLine + lineEnd)

	// Apply HTTP Header Padding if enabled (up to 21KB)
	if httpPadding > 0 {
		totalPaddingBytes := 0
		targetPadding := 21 * 1024
		if httpPadding > 21 {
			targetPadding = httpPadding * 1024
		}
		for i := 0; totalPaddingBytes < targetPadding && i < 200; i++ {
			paddingKey := fmt.Sprintf("X-Evasion-Padding-%03d", i)
			paddingVal := strings.Repeat("A", 120)
			line := paddingKey + ": " + paddingVal + lineEnd
			totalPaddingBytes += len(line)
	...
	if mutateHost {
		...
		if hostVal != "" {
			// Trailing dot: Host: domain.com. (only if not an IP address)
			if !strings.HasSuffix(hostVal, ".") && net.ParseIP(hostVal) == nil {
				hostVal = hostVal + "."
			}
			// Host casing: hOsT
			hostKey := "hOsT"
			delete(headers, foundKey)
			headers[hostKey] = []string{hostVal}
		}
	}

	// Header Order Swap: Put Connection before Host/hOsT if both exist
	order := []string{"Connection", "Host", "hOsT", "User-Agent", "Accept", "Accept-Language", "Accept-Encoding"}
	for _, key := range order {
		if vals, found := headers[key]; found {
			for _, val := range vals {
				writeHeaderLine(&reassembled, key, val, mutateHeaderSpace, lineEnd)
			}
			delete(headers, key)
		}
	}
	...
	reassembled.WriteString(lineEnd)
	reassembled.WriteString(bodyPart)

	return []byte(reassembled.String())
}

func writeHeaderLine(w *strings.Builder, key, val string, mutateHeaderSpace bool, lineEnd string) {
	if mutateHeaderSpace {
		if strings.EqualFold(key, "host") || key == "hOsT" {
			// Host tabulation: Host: domain.com\t
			// No space colon: Host:domain.com
			// Extra space before colon: Host : domain.com
			r := rand.Intn(3)
			if r == 0 {
				w.WriteString(key + ":" + val + "\t" + lineEnd)
			} else if r == 1 {
				w.WriteString(key + ":" + val + lineEnd)
			} else {
				w.WriteString(key + " : " + val + lineEnd)
...
func SniSplits(data []byte) []int {
	hostStart, hostEnd := tlsfragment.SNIHostRange(data)
