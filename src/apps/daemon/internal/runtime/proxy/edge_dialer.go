...
func (ed *EdgeDialer) dialWebSocket(ctx context.Context, targetHost string, targetPort int) (net.Conn, error) {
	...
	configureProxyWebSocket(wsConn)
...
func encodeSOCKS5Target(targetHost string, targetPort int) ([]byte, error) {
	targetHost = strings.TrimSpace(targetHost)
	if targetHost == "" || strings.IndexByte(targetHost, 0) >= 0 {
		return nil, fmt.Errorf("SOCKS5 target host is invalid")
	}
	if targetPort < 1 || targetPort > 65535 {
		return nil, fmt.Errorf("SOCKS5 target port is invalid: %d", targetPort)
	}
	...
	if ip := net.ParseIP(targetHost); ip != nil {
		...
		if len(targetHost) > 255 {
			return nil, fmt.Errorf("SOCKS5 target hostname too long: %d bytes", len(targetHost))
		}
		addrType = 0x03
		addrVal = append([]byte{byte(len(targetHost))}, []byte(targetHost)...)
	}
	...
	req[3] = addrType
	copy(req[4:], addrVal)
	binary.BigEndian.PutUint16(req[4+len(addrVal):], uint16(targetPort))
	return req, nil
}

func (ed *EdgeDialer) handshakeSOCKS5(conn net.Conn, targetHost string, targetPort int) error {
	// Encode the complete request before any network mutation so invalid
	// one-byte domain lengths or uint16 ports fail without a partial handshake.
	req, err := encodeSOCKS5Target(targetHost, targetPort)
	if err != nil {
		return err
	}
...
type edgeWSConn struct {
	*websocket.Conn
	readBuf       []byte
	isVLESS       bool
	vlessResponse vlessWSResponseState
}
...
func (c *edgeWSConn) Read(b []byte) (int, error) {
	...
	var msg []byte
	for {
		messageType, next, err := c.ReadMessage()
		if err != nil {
			return 0, err
		}
		...
		if c.isVLESS {
			var ready bool
			msg, ready, err = c.vlessResponse.consume(msg)
			if err != nil {
				return 0, err
			}
			if !ready {
...
func (c *edgeWSConn) LocalAddr() net.Addr                { return proxyWebSocketLocalAddr(c.Conn) }
func (c *edgeWSConn) RemoteAddr() net.Addr               { return proxyWebSocketRemoteAddr(c.Conn) }
func (c *edgeWSConn) SetDeadline(t time.Time) error      { return proxyWebSocketSetDeadline(c.Conn, t) }
