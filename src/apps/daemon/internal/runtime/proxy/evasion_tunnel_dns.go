...
var SecureResolverPresets = map[string]string{
	...
	"shecan-doh":         "https://free.shecan.ir/dns-query",
	"electro-doh":        "https://dns.electrotm.org/dns-query",
	"comss-doh":          "https://dns.comss.one/dns-query",
	"uncensoreddns-doh":  "https://unicast.uncensoreddns.org/dns-query",
	"joindns-protective": "https://protective.joindns4.eu/dns-query",
	"joindns-unfiltered": "https://unfiltered.joindns4.eu/dns-query",
...
func (m *EvasionTunnelManager) startDNSForwarder(port int, dnsResolver string, ctx context.Context) {
	...
	for {
		...
		go func(data []byte, addr *net.UDPAddr) {
			defer queryWG.Done()
			resp, err := m.forwardDNSQueryContext(ctx, data, dnsResolver)
			if err != nil {
				if ctx.Err() != nil {
					return
				}
...
func (m *EvasionTunnelManager) forwardDNSQuery(query []byte, resolver string) ([]byte, error) {
	return m.forwardDNSQueryContext(context.Background(), query, resolver)
}

func (m *EvasionTunnelManager) forwardDNSQueryContext(ctx context.Context, query []byte, resolver string) ([]byte, error) {
	if resolver == "" {
		resolver = "https://dns.quad9.net/dns-query"
	}

	resp, err := m.forwardDNSQuerySingleContext(ctx, query, resolver)
	if err == nil {
		return resp, nil
	}
	if ctx.Err() != nil {
	...
	for _, fb := range fallbackDnsResolvers {
		...
		resp, err = m.forwardDNSQuerySingleContext(ctx, query, fb)
		if err == nil {
...
func (m *EvasionTunnelManager) forwardDNSQuerySingle(query []byte, resolver string) ([]byte, error) {
	return m.forwardDNSQuerySingleContext(context.Background(), query, resolver)
}

func (m *EvasionTunnelManager) forwardDNSQuerySingleContext(ctx context.Context, query []byte, resolver string) ([]byte, error) {
	if strings.HasPrefix(resolver, "https://") || strings.HasPrefix(resolver, "http://") {
		client := &http.Client{Timeout: 3 * time.Second}
		req, err := http.NewRequestWithContext(ctx, "POST", resolver, bytes.NewReader(query))
		if err != nil {
			return nil, err
		}
		...
		respData, err := readBoundedProxyHTTPBody(resp.Body, maxProxyDNSHTTPBodyBytes, "DoH response")
		if err != nil {
			return nil, err
		}
	...
	conn, err := mobilehost.DialContext(ctx, "udp", dnsAddr, 2*time.Second)
	if err != nil {
		return nil, err
	}
	defer conn.Close()
...
func resolveWithResolver(host string, resolver string) ([]string, error) {
	...
	r := &net.Resolver{
		...
		Dial: func(ctx context.Context, network, address string) (net.Conn, error) {
			return mobilehost.Dialer(2*time.Second).DialContext(ctx, "udp", dnsAddr)
		},
	}
	...
	client := &http.Client{
		...
		Transport: &http.Transport{
			DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
				return mobilehost.Dial(network, addr, 3*time.Second)
			},
		},
	}
