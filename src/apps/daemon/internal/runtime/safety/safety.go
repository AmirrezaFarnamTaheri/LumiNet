...
// DefaultSettings returns the conservative policy shared by every transport adapter.
...
func NewGovernor(auditLogPath string) *Governor {
	if auditLogPath == "" {
...
func (sg *Governor) ValidateScan(targetIP string, settings Settings) error {
...
func (sg *Governor) writeAudit(target string, settings Settings, passed bool, errMsg string) {
	sg.mu.Lock()
	defer sg.mu.Unlock()
...
func (sg *Governor) AuditProcessSecurity() ([]string, error) {
	sg.mu.Lock()
	defer sg.mu.Unlock()

	var issues []string
	envVars := []string{"_DEBUG_", "COR_ENABLE_PROFILING", "LD_PRELOAD", "DYLD_INSERT_LIBRARIES"}
	for _, env := range envVars {
		if val := os.Getenv(env); val != "" {
			issues = append(issues, fmt.Sprintf("Suspicious environment variable detected: %s = %s", env, val))
		}
	}
	return issues, nil
}

var globalGovernor = NewGovernor("")

// GetGovernor returns the process-wide safety policy owner shared by adapters.
func GetGovernor() *Governor { return globalGovernor }
