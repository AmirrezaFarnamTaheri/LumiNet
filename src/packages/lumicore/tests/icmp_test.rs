...
async fn test_icmp_scan_single() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Should have exactly one target in results"
    );
    assert_eq!(
        results[0].target,
        target.to_string(),
        "Target string matches"
    );
}
...
async fn test_icmp_scan_cidr() {
    ...
    assert_eq!(
        results.len(),
        1,
        "CIDR scan should yield exactly one result"
    );
    assert_eq!(
        results[0].target, "127.0.0.1",
        "Target matches expanded CIDR"
    );
}
