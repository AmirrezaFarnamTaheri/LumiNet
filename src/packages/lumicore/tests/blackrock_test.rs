...
fn test_blackrock_uniqueness_and_reversibility() {
    ...
    for i in 0..range {
        ...
        assert!(
            shuffled < range,
            "Shuffled value {} must be within range {}",
            shuffled,
            range
        );
    ...
    assert_eq!(
        shuffled_set.len(),
        range as usize,
        "All shuffled values must be unique"
    );
}
...
fn test_blackrock_determinism() {
    ...
    for i in 0..100 {
        assert_eq!(
            br1.shuffle(i),
            br2.shuffle(i),
            "Shuffling must be deterministic"
        );
        assert_eq!(
            br1.unshuffle(i),
            br2.unshuffle(i),
            "Unshuffling must be deterministic"
        );
    }
}
