...
fn lock_sessions() -> Result<
    std::sync::MutexGuard<'static, std::collections::HashMap<u64, IosSession>>,
    String,
> {
    session_map()
        .lock()
        .map_err(|_| "iOS session state is poisoned".to_string())
}
    ...
    safe(
        ...
        AssertUnwindSafe(|| {
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    tracing::info!(
                        "ios: leaf starting rt_id={} tun_fd={} socks={}",
                        leaf_rt_id,
                        tun_fd,
                        socks_port
                    );
                    ...
                    match result {
                        Ok(Ok(())) => {
                            tracing::info!("ios: leaf rt_id={} stopped cleanly", leaf_rt_id)
                        }
                        Ok(Err(e)) => {
                            tracing::error!("ios: leaf rt_id={} error: {:?}", leaf_rt_id, e)
                        }
                        Err(_) => tracing::error!(
                            "ios: leaf rt_id={} PANICKED — caught by catch_unwind",
                            leaf_rt_id
                        ),
                    }
            ...
            tracing::info!(
                "ios: session {} started (leaf rt_id={})",
                session_id,
                leaf_rt_id
            );
            session_id
...
pub extern "C" fn mhrv_stop(session_id: u64) -> bool {
    safe(
        false,
        AssertUnwindSafe(|| {
            let mut map = match lock_sessions() {
                Ok(map) => map,
                Err(err) => {
                    tracing::error!("ios: {err}");
                    return false;
