...
fn current_ts() -> u64 {
    crate::netutil::current_timestamp()
}
...
impl IcmpScanner {
    ...
    pub async fn scan_targets(&self, targets: Vec<IpAddr>) -> LumiResult<Vec<ProbeResult>> {
        ...
        // Fixed-size worker pool based on max_concurrent setting
        let num_workers = (self.config.max_concurrent as usize).min(total).max(1);

        // Use async channels so idle workers do not block the Tokio runtime.
        let (target_tx, target_rx) = tokio::sync::mpsc::channel::<IpAddr>(num_workers * 2);
        let target_rx = Arc::new(Mutex::new(target_rx));
        // Results are unbounded because they are drained after all workers finish.
        let (result_tx, mut result_rx) = tokio::sync::mpsc::unbounded_channel::<ProbeResult>();
            ...
            let worker = tokio::spawn(async move {
                loop {
                    ...
                    // Update stats
                    {
                        ...
                        let total_sent = s.total_sent as f64;
                        if total_sent > 0.0 {
                            s.loss_rate = (s.total_timeout + s.total_error) as f64 / total_sent;
                        }
                    }
        ...
        // Drop local clones to prevent hanging
        drop(result_tx);

        // Feed targets into the pipeline with pacing
        let feeder = {
...
async fn probe_single_ip(target: IpAddr, timeout_ms: u32, retry_count: u8) -> ProbeResult {
    ...
    if !final_result.success {
        ...
        let target_str_80 = target_str.clone();
        let tcp_res_80 = crate::ffi::exports::block_on_compio(async move {
            crate::tcp::tcp_connect(&target_str_80, 80, timeout_ms).await
        });
        if let Ok(Ok(tcp_res)) = tcp_res_80 {
            if tcp_res.success {
                let mut res = tcp_res;
                res.metadata
                    .insert("fallback".to_string(), "tcp_80".to_string());
                return res;
            }
        }
        let target_str_443 = target_str.clone();
        let tcp_res_443 = crate::ffi::exports::block_on_compio(async move {
            crate::tcp::tcp_connect(&target_str_443, 443, timeout_ms).await
        });
        if let Ok(Ok(tcp_res)) = tcp_res_443 {
            if tcp_res.success {
                let mut res = tcp_res;
                res.metadata
                    .insert("fallback".to_string(), "tcp_443".to_string());
                return res;
            }
        }
    }
