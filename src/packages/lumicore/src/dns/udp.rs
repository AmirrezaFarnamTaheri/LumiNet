...
use crate::dns::packet::{build_query, parse_response, response_matches_query, TYPE_A};
...
pub async fn resolve(
    ...
    if !response_matches_query(&buf, domain, record_type, txid)? {
        return Err("DNS response did not match the requested transaction and question".into());
    }
