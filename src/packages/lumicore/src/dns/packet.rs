...
//!
//! Hand-crafted DNS packet construction and parsing. Builds raw DNS query packets and parses
...
/// Verifies that a DNS response belongs to the exact query that was sent.
///
/// Transaction IDs alone are insufficient because spoofed or stale packets can
/// reuse an ID. The echoed question must also match the requested name, type,
/// and Internet class, and the packet must be marked as a response.
pub fn response_matches_query(
    buf: &[u8],
    domain: &str,
    record_type: u16,
    transaction_id: u16,
    ...
    let (question_name, consumed) = decode_domain_name(buf, 12)?;
    let offset = 12 + consumed;
    if offset + 4 > buf.len() {
        return Ok(false);
    }
    let question_type = u16::from_be_bytes([buf[offset], buf[offset + 1]]);
    let question_class = u16::from_be_bytes([buf[offset + 2], buf[offset + 3]]);
    let expected_name = domain.trim_end_matches('.');

    Ok(question_name.eq_ignore_ascii_case(expected_name)
...
pub fn parse_response(buf: &[u8]) -> Result<Vec<DnsRecord>, Box<dyn std::error::Error>> {
    ...
    for _ in 0..ancount {
        ...
        let (type_str, value) = match rtype {
            ...
            TYPE_HTTPS => {
                if rdata.len() < 2 {
                    ("HTTPS".to_string(), format!("Data: {}", hex::encode(rdata)))
                } else {
...
#[cfg(test)]
mod query_match_tests {
    use super::*;

    fn response_for(domain: &str, record_type: u16, txid: u16) -> Vec<u8> {
        let mut packet = build_query(domain, record_type, txid);
        packet[2] |= 0x80;
        packet
    }

    #[test]
    fn response_match_requires_full_echoed_question() {
        let packet = response_for("Example.COM", TYPE_A, 0x1234);
        assert!(response_matches_query(&packet, "example.com", TYPE_A, 0x1234).unwrap());
        assert!(!response_matches_query(&packet, "other.example", TYPE_A, 0x1234).unwrap());
        assert!(!response_matches_query(&packet, "example.com", TYPE_AAAA, 0x1234).unwrap());
        assert!(!response_matches_query(&packet, "example.com", TYPE_A, 0x4321).unwrap());
    }

    #[test]
    fn response_match_rejects_query_packets_and_invalid_question_count() {
        let mut packet = build_query("example.com", TYPE_A, 7);
        assert!(!response_matches_query(&packet, "example.com", TYPE_A, 7).unwrap());
        packet[2] |= 0x80;
        packet[4] = 0;
        packet[5] = 0;
        assert!(!response_matches_query(&packet, "example.com", TYPE_A, 7).unwrap());
