...
pub fn send_fake_packet(
    ...
    // For IPv6, the kernel handles the pseudo-header checksum on raw sockets,
    // so we only compute it for IPv4.
    let mut header_bytes = header.to_bytes();
    match (local_ip, target_ip) {
        (IpAddr::V4(src), IpAddr::V4(dst)) => {
            let checksum = header.calculate_checksum(src.octets(), dst.octets(), payload);
            header_bytes[16..18].copy_from_slice(&checksum.to_be_bytes());
        }
        _ => {
            // IPv6: leave checksum as 0; the kernel fills in the pseudo-header checksum
            // for IPPROTO_TCP raw sockets via IPV6_CHECKSUM socket option.
        }
    }
