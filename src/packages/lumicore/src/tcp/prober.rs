...
pub async fn tcp_connect(
    ...
    // Resolve address
    let ip = match addr_str.to_socket_addrs()?.next() {
    ...
    // Connect to target using compio proactor
    let connect_future = compio::net::TcpStream::connect(socket_addr);
    let result =
        compio::time::timeout(Duration::from_millis(timeout_ms as u64), connect_future).await;

    let latency = start.elapsed().as_secs_f64() * 1000.0;
    let timestamp = current_timestamp();

    match result {
        Ok(Ok(_stream)) => Ok(ProbeResult {
...
pub async fn banner_grab(
    ...
    let write_future = stream.write_all(b"\r\n\r\n");
    let _ = compio::time::timeout(Duration::from_millis(timeout_ms as u64), write_future).await;

    let buffer = vec![0u8; 1024];
    let read_future = stream.read(buffer);
    let timeout_res =
        compio::time::timeout(Duration::from_millis(timeout_ms as u64), read_future).await;
    let compio::BufResult(bytes_read_res, buffer) = match timeout_res {
        Ok(buf_res) => buf_res,
        Err(_) => {
            return Err(Box::new(std::io::Error::new(
                std::io::ErrorKind::TimedOut,
                "Timed out reading banner",
            )))
        }
    };
