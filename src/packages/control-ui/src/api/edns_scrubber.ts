...
export class EdnsSubnetScrubber {
  constructor(
    public maskIpv4Bits: number = 24,
    public maskIpv6Bits: number = 56,
    public stripCompletely: boolean = false
  ) {}
  ...
  maskIpv4(ip: string): string {
    ...
    const mask = this.maskIpv4Bits >= 32 ? 0xffffffff : this.maskIpv4Bits <= 0 ? 0 : (~((1 << (32 - this.maskIpv4Bits)) - 1)) >>> 0;
