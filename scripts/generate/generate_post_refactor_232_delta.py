...
BASE=Path(os.environ.get('LUMINET_232_BASELINE_INVENTORY','/mnt/data/LumiNet-post-refactor-231-source-inventory.csv'))
OUT=ROOT/'governance/convergence/post-refactor-232-target-delta.csv'
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
def current():
 ...
 for p in sorted(ROOT.rglob('*')):
  ...
  if rel=='governance/convergence/post-refactor-232-target-delta.csv':continue
...
print('post-refactor-232 delta',len(rows),dict(Counter(r['change_type'] for r in rows)))
