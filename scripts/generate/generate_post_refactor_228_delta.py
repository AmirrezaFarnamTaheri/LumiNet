...
BASE=ROOT/'governance/convergence/post-refactor-228-baseline-files.csv'
OUT=ROOT/'governance/convergence/post-refactor-228-target-delta.csv'
...
def mode(p:Path)->str:return oct(stat.S_IMODE(p.stat().st_mode))
with BASE.open(newline='',encoding='utf-8') as f:base={r['path']:r for r in csv.DictReader(f)}
cur={}
for p in sorted(ROOT.rglob('*')):
 if not p.is_file() or p.is_symlink():continue
 rel=p.relative_to(ROOT).as_posix()
 if rel==SELF or rel.startswith('.git/') or '/node_modules/' in '/'+rel or '__pycache__' in rel:continue
...
fields=['change_type','path','baseline_sha256','current_sha256','baseline_size_bytes','current_size_bytes','baseline_mode','current_mode']
