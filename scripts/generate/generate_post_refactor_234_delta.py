...
OUT=ROOT/'governance/convergence/post-refactor-234-target-delta.csv'
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
for p in sorted(ROOT.rglob('*')):
 ...
 if rel=='governance/convergence/post-refactor-234-target-delta.csv' or '/node_modules/' in '/'+rel or '__pycache__' in rel or rel.startswith('.git/'):continue
...
print('post-refactor-234 delta',len(rows),dict(Counter(r['change_type'] for r in rows)))
