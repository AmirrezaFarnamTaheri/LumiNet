...
assert_command "air" "go install github.com/air-verse/air@v1.67.3"
if [ "$SKIP_WEB" = false ]; then
    assert_command "npm" "https://nodejs.org/"
    assert_command "node" "https://nodejs.org/"
fi
if [ "$SKIP_RUST_WATCH" = false ]; then
    assert_command "cargo" "https://rustup.rs/"
    assert_command "cargo-watch" "cargo install cargo-watch --version 8.5.3 --locked"
fi
...
export CGO_LDFLAGS="$(python3 "$LINK_RESOLVER" --profile debug --target x86_64-pc-windows-gnu)"
echo -e "\033[0;32m  ✅ Rust core ready\033[0m"

# ── Start Vite Dev Server ─────────────────────────────────────────────────
if [ "$SKIP_WEB" = false ]; then
    echo -e "\033[0;36m  ⚛️  Starting Vite dev server on :$PORT...\033[0m"
    (cd "$WEB_DIR" && npm run dev -- --port "$PORT") &
    PIDS+=($!)
fi
...
if [ "$SKIP_RUST_WATCH" = false ]; then
    ...
    (cd "$CORE_DIR" && cargo watch -w src -s "cargo build") &
    PIDS+=($!)
fi
