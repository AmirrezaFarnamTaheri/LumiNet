...
unreachable = [p for p in api_files if p not in reachable]
errors: list[str] = []
if unreachable:
    errors.append(
...
if manifest.get("api_total") != len(api_files):
    errors.append(f"authority manifest api_total={manifest.get('api_total')} but canonical src/api has {len(api_files)} modules")
if manifest.get("api_reachable") != len(api_files):
...
print(f"control-ui-api-authority canonical={len(api_files)} unreachable={len(unreachable)} errors={len(errors)}")
for error in errors:
    print(f"ERROR: {error}")
