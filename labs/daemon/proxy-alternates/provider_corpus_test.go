...
func TestProviderCorpusAdaptersPreservePriorityAndStaleStatus(t *testing.T) {
	corpus, err := ParseProviderCorpus([]byte(`{
		"schema_version": 1,
		"corpus_id": "adapter-test",
		...
		"checksum": "test",
		"providers": [
			{"provider_id":"lower-priority","priority":1,"ipv4_prefixes":["198.51.100.0/24"]},
			{"provider_id":"higher-priority","priority":2,"ipv4_prefixes":["198.51.100.0/24"]}
		]
	}`))
	if err != nil {
		t.Fatal(err)
	}
	if err := UpdateProviderCorpusRegistry(corpus); err != nil {
		t.Fatal(err)
	}
	observation := ObserveProvider("198.51.100.1")
	if observation.ProviderID != "higher-priority" || observation.Source != "provider_corpus_store" || observation.Prefix != "198.51.100.0/24" || observation.CorpusID != "adapter-test" {
		t.Fatalf("observation=%+v", observation)
	}
	status, ok := GetProviderCorpusStoreStatus()
	if !ok || !status.Stale || status.SchemaVersion != 1 || status.FetchedAt == "" || status.Checksum != "test" {
		t.Fatalf("status=%+v ok=%v", status, ok)
	}
	classification, ok := MatchRadixClassification(netip.MustParseAddr("198.51.100.1"))
	if !ok || classification != "higher-priority" {
		t.Fatalf("classification=%q ok=%v", classification, ok)
	}
}
